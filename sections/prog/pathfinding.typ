...
== 10.4.1   Step #1: Aligning with the tile's center

We can use the integrated PID-based Lemlib's moveToPoint function, but we need the arguments to fill into it: x and y coordinates. In step 1, the coordinates are always the center of a foam tile, so we can divide every tile in 3 types: *Achievable, Non-achievable and Semi-achievable*:

	A tile is considered *achievable*, if from every point on the same tile, where a robot can theoretically end up, there exists a path in the shape of a straight line to move into its center (desired point in step 1).

	A tile is considered *semi-achievable*,if it does not fall into the achievable criteria, but its center is unobstructed and a robot satisfying the size regarding rules(R5, SG2, SG3) can be placed into it manually, from a certain direction.

	A tile is considered *non-achievable* if and only if, a robot of a maximum legal size cannot be placed into the center of that tile without physically engaging with the environment (Center/long goals).



#figure(
  image("../../Photos/Screenshot 2026-09-17 at 17.39.28.png")
)
 *Here's a map of every tile based on its achievability. The canter tiles are marked red, as their center is obstructed by the middle goal, therefore can't be placed into the center without touching the goal.*

*The tiles underneath the parking zone are technically considered semi-achievable, but in a real match, engaging with the parking zone is risky, as it is not as consistent as normal driving, and possesses a risk of lifting of the ground, completely breaking the odometry, therefore will be treated as unachievable.*
